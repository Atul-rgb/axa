...
public class TC_PATestAdjustment_001_newTC extends BaseClass  {
	String parent;
	String child;
	String paNdetails;
	String actions;
	...
	public void paTestAdjustment(String row) throws IOException
	{
		try {
			...
			//===
			if(paread.getPaManual(currentRow, "Pending").equalsIgnoreCase("No")) {
			if (paread.getPaAdjustment(currentRow, "ActionView").equals("Yes")) {
				preApproval_PA_List_Page.check_go_to_previous_page_display();
				logger.info("go_to_previous_page is display");
                preApproval_PA_List_Page.check_view_Letter();
                logger.info("View letter icon is display");
                preApproval_PA_List_Page.check_view_Log();
                logger.info("View log icon is display");
                preApproval_PA_List_Page.check_nextButton();
                logger.info("Next button is display");
                preApproval_PA_List_Page.check_pre_approval_link();
                logger.info("Pre Approval is display");
			...
			logger.info("Adjustment icon clicked");
				...
				pAListPage.clickproceed();
              logger.info("Click Procced link");
			}
			...
			logger.info("Approve Clicked...");
				...
				pa.clickappRove_pending();
				logger.info("Approve Clicked for pending process...");
				pop_up_Reason_For_Aprove.click_dropdown();
				logger.info(" dropdown Clicked for pending process...");
				pop_up_Reason_For_Aprove.click_checkbox_selectall();
				logger.info("CheckBox Clicked for pending process...");
				pop_up_Reason_For_Aprove.click_submit_button();
				logger.info(" Clicked submit button for pending process...");
			}
			//==================
	...
	@Parameters(value="tc_row")
	@Test(priority=4, dependsOnMethods = { "palistSearchAdjustment" })
